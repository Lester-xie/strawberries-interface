...
import { Currency, currencyEquals, ETHER, Percent } from '../../sdk'
import {WETH,KLPNAME} from '../../constants'
...
const Body = styled.div`
  ...
  padding-right: 24px;
  .dFqEWm {
    background-color: rgba(229,82,104,0.1);
  }
`
    ...
    return (
      <AutoColumn gap="md" style={{ marginTop: '20px' }}>
        ...
        <Italic fontSize={12} color={theme.colors.textSubtle} textAlign="left" padding="12px 0 0 0">
          {TranslateString(1346, `Output is estimated. If the price changes by more than ${
            allowedSlippage / 100
          }% your transaction will revert.`,{rota: `${allowedSlippage / 100}%`})}
    ...
    return (
      <>
        ...
        {pair && (
          <>
            <RowBetween>
              <Text color="textSubtle">{TranslateString(928, 'Price')}</Text>
              <Text>
          ...
          <TransactionConfirmationModal
            ...
            content={() => (
              <ConfirmationModalContent
                title={TranslateString(906, 'You will receive')}
                onDismiss={handleDismissConfirmation}
          ...
          <AutoColumn gap="md">
            <Body>
              <OutlineCard>
                <AutoColumn>
                  <RowBetween>
                    <Text>{TranslateString(9999, 'Amount')}</Text>
                    <ClickableText
                      onClick={() => {
                        setShowDetailed(!showDetailed)
                      }}
                    >
                      {showDetailed ? TranslateString(930, 'Simple') : TranslateString(932, 'Detailed')}
                    </ClickableText>
                  </RowBetween>
                  ...
                  {!showDetailed && (
                    <>
                      ...
                      <Flex justifyContent="space-between">
                        <SliderBtn variant="tertiary" size="sm" onClick={() => onUserInput(Field.LIQUIDITY_PERCENT, '25')}>
                          25%
                        </SliderBtn>
                        <SliderBtn variant="tertiary" size="sm" onClick={() => onUserInput(Field.LIQUIDITY_PERCENT, '50')}>
                          50%
                        </SliderBtn>
                        <SliderBtn variant="tertiary" size="sm" onClick={() => onUserInput(Field.LIQUIDITY_PERCENT, '75')}>
                          75%
                        </SliderBtn>
                        <SliderBtn
                          variant="tertiary"
                      ...
                      {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                        <RowBetween style={{ justifyContent: 'flex-end' }}>
                          {oneCurrencyIsETH ? (
                            ...
                            >
                              {TranslateString(934, 'Receive WOKT')}
                            </StyledInternalLink>
                          ) : oneCurrencyIsWETH ? (
                            <StyledInternalLink
                              to={`/remove/${
                            ...
                            >
                              {TranslateString(936, 'Receive OKT')}
                            </StyledInternalLink>
                          ) : null}
                        </RowBetween>
                      ) : null}
                  ...
                  <RowBetween>
                    ...
                    >
                      {approval === ApprovalState.PENDING ? (
                        <Dots>{TranslateString(804, 'Approving')}</Dots>
                      ) : approval === ApprovalState.APPROVED || signatureData !== null ? (

                        TranslateString(806, 'Approved')
                      ) : (
                        TranslateString(1438, 'Approve')
                      )}
                    </Button>
                    <Button
                      style={{width: '50%'}}
                    ...
                    >
                      {error || TranslateString(860,'Remove')}
                    </Button>
                  </RowBetween>
                )}
              </div>
